...
def goal_track(img, bbox):
    ...
    c1 = x+int(w/2)
    c2 = y+int(h/2)
    cv2.circle(img,(c1,c2),2,(145,67,72),5)
    cv2.circle(img,(int(p1),int(p2)),2,(145,67,72),5)
    distance = math.sqrt(((c1-p1)**2)+(c2-p2)**2)
    print(distance)
    if(distance<=20):
        cv2.putText(img,"Scored",(300,90),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.7,(0,255,0),2)
    xs.append(c1)
    ys.append(c2)
    for i in range(len(xs)-1):
        cv2.circle(img,(xs[i],ys[i]),2,(123,123,123),5)
